...
export const TodoList: React.FC<Props> = ({ totalCount, onAdd }) => {
  ...
  const swapItems = (): void => {
    ...
    const currentTranslate =
      targetDragNodeCurrent.style.transform.match(/-?\d+/g)?.[0] ?? 0;
    const targetTranslate =
      dragNodeCurrent.style.transform.match(/-?\d+/g)?.[0] ?? 0;
    ...
    if (topDragNode > topTargetDragNode) {
      targetDragNodeCurrent.style.transform = `translateY(${
        Number(currentTranslate) + 100
      }%)`;
      dragNodeCurrent.style.transform = `translateY(${
        Number(targetTranslate) - 100
      }%)`;
    } else {
      targetDragNodeCurrent.style.transform = `translateY(${
        Number(currentTranslate) - 100
  ...
  const reorderList = (): void => {
    ...
    const targetIndex = _todos.findIndex(
      (todo) => todo.id === targetDragIdCurrent
    );
